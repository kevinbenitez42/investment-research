...
def normalize_nasdaq_100_constituents(tables: list[pd.DataFrame]) -> pd.DataFrame:
    ...
    try:
        ...
        sector_column = next(column for column in table.columns if str(column).startswith("ICB Industry"))
        subsector_column = next(column for column in table.columns if str(column).startswith("ICB Subsector"))
    except ValueError:
        table = find_table(
            tables,
        ...
        subsector_column = "ICB Industry"

    return table[["Ticker", "Company", sector_column, subsector_column]].rename(
        columns={
            "Ticker": "Symbol",
            sector_column: "Sector",
            subsector_column: "Sub-Industry",
        }
    )
