...
from Quantapp.visualization.traces.candlestick import (
...
def _build_underwater_trace(window, x, y, *, visible):
...
def _build_textbook_drawdown_trace(window, x, y, *, visible):
    return build_line_trace(
        x=x,
        y=y,
...
def _build_recovery_time_trace(window, x, y, *, visible):
    return build_line_trace(
        x=x,
        y=y,
...
def _build_percentile_reference_trace(
    *,
    x,
    y_value,
    name,
...
def plot_candlestick_drawdown_recovery_view(
    ...
    for window in window_options:
        ...
        fig.add_trace(
            _build_underwater_trace(window, visible_underwater.index, visible_underwater, visible=window_visible),
            row=2,
            col=1,
        )
        ...
        fig.add_trace(
            _build_textbook_drawdown_trace(window, visible_drawdown.index, visible_drawdown, visible=window_visible),
            row=2,
            col=1,
        )
        lower_panel_trace_indices.append(len(fig.data) - 1)
        ...
        fig.add_trace(
            _build_percentile_reference_trace(
                x=drawdown_x_ref,
        ...
        fig.add_trace(
            _build_recovery_time_trace(window, visible_recovery.index, visible_recovery, visible=window_visible),
            row=3,
            col=1,
        )
        lower_panel_trace_indices.append(len(fig.data) - 1)
